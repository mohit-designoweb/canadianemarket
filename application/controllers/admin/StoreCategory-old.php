...
class StoreCategory extends CI_Controller {
    ...
    public function doEditCategory($id) {
        ...
        if ($result) {
            $this->output->set_output(json_encode(['result' => 1, 'url' => base_url('admin/category')]));
            return FALSE;
        } else {
            $this->output->set_output(json_encode(['result' => -1, 'msg' => 'No Changes Were Made.', 'url' => base_url('category/category')]));
            return FALSE;
        }
    }
