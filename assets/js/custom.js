...
$('#cash_delivery').click(function(){
    var radIn = $(this).is(':checked');
    if(radIn){
       $(this).parents('div.cashBx').prev('div.width40').find('.posrel').children('input').prop("disabled",true);
        $(this).parents('div.cashBx').prevAll('.reswidth100').find('.card_drop_down').children('.carddropbtn').prop("disabled",true);
         $(this).parents('div.cashBx').prevAll('.inbdsbl').find('.form-group').children('input').prop("disabled",true);

    }else {
       $(this).parents('div.cashBx').prev('div.width40').find('.posrel').children('input').prop("disabled",false); 
    }
}); 

$('#online_payment').click(function() {
   var radinp = $(this).is(':checked');
   if(radinp) {
       $(this).parents('div.payment_lft').find('input').prop("disabled",false);
       $(this).parents('div.payment_lft').find('button').prop("disabled",false);
   }
});
...
$('.drop_menu_s3 li a').click(function () {
    ...
    $(this).parents('.drop_menu').slideUp();
    $(this).parents('.drop_down').find('.dropbtn3').removeClass('active');
    $(this).parents('.drop_down').find('img.rotate').removeClass('rotatearrow');
});
$('.drop_menu_s3 li a').click(function () {
    var elms = $(this).text();
...
var count = 1;
$(document).on('click', '.vaulebox .plus_btn', function () {
    var result1 = (count >= 1) ? count++ : 1;
    $(this).prev(' input').val(result1);
});

$(document).on('click', '.vaulebox .minus_btn', function () {
    var result = (count <= 1) ? 1 : count--;
...
$(document).on('click', '.cate_drop_menu_s2 li a', function () {
    var elms = $(this).text();
...
$(document).on('click', '.cate_drop_menu_s3 li a', function () {
    var elms = $(this).text();
...
$(document).on('click', '.filDrop_btn', function () {
...
$(document).on('click', '.slides_food li', function () {
...
$(document).on('click', '.slides button.arrow_up', function () {
...
$(document).on('click', '.slides button.arrow_down', function () {
    var cP = $('.slides').height();
    var eP = $('.slides ul').height();
    var lH = $('.slides ul li').height() - 10;
    $('.slides ul').css('transform', 'translateY(' + lH + 'px)');
...
// $(function () {
//     $(".morebtn").slice(0, 7).show().css('display','block');
//             $("#load").on('click', function (e) {
//             e.preventDefault();
//         $(".morebtn:hidden").css('display','block').slice(0, 5).slideDown();
//             if ($(".morebtn:hidden").length == 0) {
//                 $("#load").fadeOut('slow');
//             }
//         //     $('html,body').animate({
